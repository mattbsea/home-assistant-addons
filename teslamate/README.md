> **Note (mattbsea fork):** Vendored copy of [lildude/ha-addon-teslamate](https://github.com/lildude/ha-addon-teslamate) with [PR #142](https://github.com/lildude/ha-addon-teslamate/pull/142) applied (TeslaMate **4.0.0**) and the prebuilt `image:` removed so it **builds on-device**. Purpose: run TeslaMate v4 (native Fleet API) before the upstream add-on releases it. Remove this folder and switch back to the upstream `6be3d3da_teslamate` add-on once it ships v4.
